...
export function Booking({ setCurrentPage, selectedCourseId, setSelectedCourseId, currentUser, pendingRegistration, setPendingRegistration, onLoginSuccess }: BookingProps) {
  ...
  // Auto-scroll to top and prefill authenticated or newly-registered user details
  useEffect(() => {
    window.scrollTo(0, 0);
    if (currentUser) {
      ...
      setFullName(pendingRegistration.fullName);
      setPhone(pendingRegistration.phone);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!pendingRegistration && !currentUser) {
      alert("Please sign up before booking so we can finalize your registration after payment.");
      return;
    }
  ...
  const handlePaystackPayment = async (amountInNaira: number) => {
    ...
    try {
      try {
        ...
        const safePendingRegistration = pendingRegistration
          ? {
        ...
        const pendingCheckout = {
          booking: payload,
          pendingRegistration: safePendingRegistration,
          amount: amountInNaira,
      ...
      await openPaystackInlineCheckout(
        ...
        async (response) => {
          try {
            const verification = await verifyPaymentStatus(response.reference);
            ...
            localStorage.setItem('falcon_last_booking_success', JSON.stringify(bookingRecord));
            localStorage.setItem('falcon_last_payment_success', JSON.stringify({
              reference: response.reference,
              amount: amountInNaira,
            ...
            setPaymentError(verifyErr.message || 'Payment completed but verification failed.');
            setIsPaymentLoading(false);
          }
        },
        () => {
          setIsPaymentLoading(false);
          setPaymentError('Payment window was closed before completion. Please try again.');
        }
      );
    } catch (err: any) {
      console.error('Inline Paystack checkout failed:', err);
      setPaymentError(err.message || 'Unable to start Paystack payment.');
      setIsPaymentLoading(false);
    }
  };
