...
export function Auth({ setCurrentPage, onLoginSuccess, initialView }: AuthProps) {
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await authService.signUp(fullName, phone, email, password);
      
      setVerificationEmail(email);
      
      if (res.requiresVerification) {
        setSuccessMessage("Account registered successfully! A secure confirmation link has been sent to your email.");
        setTimeout(() => {
          setView('verify');
        }, 1500);
      } else {
